...
void *lander(void *data)
{
    ...
    // the getaddr and mksocket methods in libnet.c  are used to
    // get the address and open the socket
...
{
   FILE  *dlog =  fopen("datalog.csv", "w+");
	while(true) {
        sem_wait(&logdata);
    fprintf (dlog, "Fuel: %f, Thrust: %f, Altitude: %f,  Contact: %d, X position: %f,  Y position: %f,  Orientation: %f,  X velocity: %f,  Y velocity: %f,  Rotation: %f\n",
    landercond.fuel, 
...
int main(int argc, char *argv[])
{
    ...
    if((e = pthread_create(&dlogger, NULL, datalogging, NULL)))
